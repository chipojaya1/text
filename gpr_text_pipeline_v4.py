...
- gpr_sentences_country.csv: GPR sentences linked to countries
...
def build_sentence_level_dataset(xml_dir: str,
                                  ...
                                  term_to_category: Dict[str, str]) -> pd.DataFrame:
    """
    Process all XML transcripts and build sentence-level dataset.
    
    For each sentence, determines:
    - Whether it's a GPR sentence (contains GPR keyword)
    - Which GPR terms it contains
    - Which countries it mentions
    - Whether it contains a modifier term
    """
    rows = []
    ...
    for i, xml_path in enumerate(xml_files):
        ...
        for sent_idx, sentence in enumerate(sentences, start=1):
            # Check for GPR content
            is_gpr = sentence_contains_gpr(sentence, gpr_regex)
            gpr_terms_found = find_gpr_terms_in_sentence(sentence, gpr_regex) if is_gpr else []
            
            # Get categories of GPR terms found
            categories_found = list(set(
                term_to_category.get(t, "Unknown") for t in gpr_terms_found
            ))
            
            # Check for modifier terms
            has_modifier = sentence_contains_modifier(sentence, modifier_regex)
            
            # Find country mentions
            countries_mentioned = find_country_mentions(
                sentence, variant_to_country, country_regex
            )
            ...
            rows.append({
                ...
                "is_gpr_sentence": int(is_gpr),
                "gpr_terms": "|".join(gpr_terms_found) if gpr_terms_found else "",
                "gpr_categories": "|".join(categories_found) if categories_found else "",
                "has_modifier": int(has_modifier),
                "countries_mentioned": "|".join(sorted(countries_mentioned)) if countries_mentioned else "",
                "country_count": len(countries_mentioned)
            })
...
def build_gpr_sentence_country_level(df_sent: pd.DataFrame) -> pd.DataFrame:
    """
...
def build_firm_year_indices_simple(df_sent: pd.DataFrame,
    ...
    result["gpr_sentence_count"] = result["gpr_sentence_count"].fillna(0).astype(int)
    result["gpr_word_count"] = result["gpr_word_count"].fillna(0).astype(int)
    
    # Calculate indices
    result["I_GPR_it"] = (result["gpr_sentence_count"] > 0).astype(int)
    result["Frac_GPR_it"] = result["gpr_sentence_count"] / result["total_sentences"]
...
def main():
    ...
    n_gpr_sentences = df_sent["is_gpr_sentence"].sum()
    print(f"  Transcripts: {n_transcripts}")
    print(f"  Total sentences: {n_sentences}")
    print(f"  GPR sentences: {n_gpr_sentences} ({100*n_gpr_sentences/n_sentences:.2f}%)")
    
    # Build GPR sentence-country dataset
    print("\n[4/5] Building aggregated datasets...")
    df_gpr_country = build_gpr_sentence_country_level(df_sent)
    ...
    df_gpr_country.to_csv(os.path.join(OUTPUT_DIR, "gpr_sentences_country.csv"), index=False)
    print(f"  Saved: gpr_sentences_country.csv")
    ...
    if not df_fcy.empty:
        ...
        top_countries = df_fcy.groupby("country")["gpr_sentence_count"].sum().nlargest(10)
